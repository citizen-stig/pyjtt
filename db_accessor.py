...
class DBAccessor(object):
    ...
    CREATE_WORKLOGS_TABLE_SQL = """CREATE TABLE IF NOT EXISTS {worklog}
                        (worklog_id INTEGER PRIMARY KEY,
                         ...
                         end_date TIMESTAMP)""".format(worklog=WORKLOG_TABLE_NAME)
    ...
    def remove_worklog_entry(self, worklog_entry):
        db_conn, cursor = self._connect_to_db()
        cursor.execute('DELETE FROM {worklog} WHERE worklog_id = ?'.format(worklog=self.WORKLOG_TABLE_NAME),
                       (worklog_entry.worklog_id,))
        db_conn.commit()
        db_conn.close()
    ...
    def get_day_worklog(self, day):
        ...
        cursor.execute('SELECT '
                       ...
                       'WHERE '
                       '{worklog}.start_date LIKE (?) '.format(issues=self.ISSUES_TABLE_NAME,
